...
class Components
{
    ...
    public static function create_labels($name = ''){
        ...
        $labels = array(
            ...
            'add_new' => 'Agregar nueva ' . ucfirst($name),
            'add_new_item' => 'Agregar nueva ' . ucfirst($name),
            'edit_item' => 'Editar '. ucfirst($name),
            'new_item' => 'Nueva ' . ucfirst($name),
            'view_item' => 'Ver ' . ucfirst($name),
            'search_items' => 'Buscar ' . ucfirst($name) . 's',
            'not_found' => 'No ' . $name . 's encontradas',
            'not_found_in_trash' => 'No ' . $name . 's encontradas en la basura',
            'parent_item_colon' => ucfirst($name) . ' padre:',
        );
